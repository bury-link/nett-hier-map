(() => {
  ...
  function showChoice() {
    ...
    dialog.innerHTML = `<div class="consent-card"><p class="eyebrow">DATENSCHUTZ</p><h2 id="consent-title">Ihre Privatsphäre</h2><p>Für Karte, automatische Ortsbeschreibungen, Uploads und Meldungen nutzen wir technisch notwendige Dienste: OpenStreetMap und Friendly Captcha. Wenn Sie alle Dienste akzeptieren, laden wir zusätzlich Google Fonts für die Schrift Space Grotesk und Google Analytics zur statistischen Auswertung.</p><p class="consent-small">Bei der Nutzung der Karte, der automatischen Ortsbeschreibung und des Captchas können Daten, insbesondere Ihre IP-Adresse oder der gerundete Fundort, an die jeweiligen Anbieter übertragen werden. Ihre Einwilligung für Google Analytics können Sie jederzeit über die Datenschutz-Einstellungen ändern oder widerrufen.</p><div class="consent-actions"><button id="consent-all" class="primary-button" type="button">Alle Dienste akzeptieren <span aria-hidden="true">✓</span></button><button id="consent-essential" class="secondary-button" type="button">Nur notwendige Dienste</button></div></div>`;
