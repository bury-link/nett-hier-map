...
declare const L: any;
type Sighting = { id: string; latitude: number; longitude: number; imageUrl: string; createdAt: string };
...
async function loadDetail(): Promise<void> {
  ...
  try { const response = await fetch('/api/sightings', { headers: { Accept: 'application/json' } }); if (!response.ok) throw new Error(); const body = await response.json() as { sightings?: Sighting[] }; const sighting = body.sightings?.find((item) => item.id === id); if (!sighting) return showError('Dieser Fundort wurde entfernt oder existiert nicht.');
    ...
    const report = document.createElement('section'); report.className = 'report-block'; report.innerHTML = '<h2>Fundort melden</h2><p>Hinweise auf unzutreffende oder problematische Einträge werden von uns geprüft.</p>'; report.append(createReportForm(sighting.id)); article.append(intro, image, mapBlock, report); root.append(article);
