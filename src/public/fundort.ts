...
type Sighting = { id: string; latitude: number; longitude: number; locationSource: LocationSource; locationDescription: string | null; imageUrl: string; createdAt: string };
...
async function loadDetail(): Promise<void> {
  ...
  try { const response = await fetch('/api/sightings', { headers: { Accept: 'application/json' } }); if (!response.ok) throw new Error(); const body = await response.json() as { sightings?: Sighting[] }; const sighting = body.sightings?.find((item) => item.id === id); if (!sighting) return showError('Dieser Fundort wurde entfernt oder existiert nicht.');
    ...
    const report = document.createElement('details'); report.className = 'report-block'; report.innerHTML = '<summary>Fundort melden <span aria-hidden="true">+</span></summary><div class="report-content"><p>Hinweise auf unzutreffende oder problematische Einträge werden von uns geprüft.</p></div>'; report.querySelector('.report-content')!.append(createReportForm(sighting.id)); article.append(image, mapBlock, intro, report); root.append(article);
