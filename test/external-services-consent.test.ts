...
test('necessary services are loaded locally or on use while analytics stays consent-gated', () => {
  ...
  assert.doesNotMatch(consent, /netthier_external_services/);
  assert.match(consent, /G-0R00EL14X2/);
  assert.match(consent, /Friendly Captcha/);
  assert.match(consent, /function enableNecessaryServices\(\)/);
